...
for (slc_ssp in unique(totalmass_year_data$ssp)) {
  ...
  ggsave(here::here("sandbox",
                    sprintf("Figure-3-TS-ice-%s.png", slc_ssp)), width = 7, height = 5)
...
for (slc_ssp in unique(totalmass_year_data$ssp)) {
  ...
  ggsave(here::here("sandbox",
                    sprintf("Figure-4-year-degradation-%s.png", slc_ssp)), width = 7, height = 5)
}

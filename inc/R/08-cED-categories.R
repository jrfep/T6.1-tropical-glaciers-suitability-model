...
if (exists("gis.out")) {
    ...
    out.file <- sprintf("%s/collapse-trajectory-data.rds", target.dir)

} else {
    target.dir <- "sandbox"
    rds.file <- here::here(target.dir, "massbalance-model-data-all-groups.rds")
    out.file <- here::here(target.dir, "collapse-trajectory-data.rds")
}
...
collapse_trajectories <- foreach (
    ...
    collapse_year <- RSa_data %>% filter(RS==1) %>% summarise(collapse_year=min(year)) 
    ...
    mutate(
        ...
        countdown = if_else(is.na(collapse_year), 2100 - year, collapse_year - year)
    ) %>%
        select(unit_name, scenario, year, countdown, model_nr, RS:AUC_cED, state) 
...
saveRDS(file=out.file, collapse_trajectories)

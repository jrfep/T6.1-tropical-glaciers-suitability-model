...
```{r}
RSts <- function(years, values, init_year = 2000, collapse_threshold=0) {
  ...
  MD <- IV - CT
  RS <- OD/MD
  return(tibble(year=years[years>init_year],IV,FV,CT,OD,MD,RS))
}
```
...
RSvals <- totalmass_year_data %>% 
  filter(unit_name %in% "Cordillera de Merida",scn == "SSP1-2.6") %>%
  split(.$model_nr) %>%

...
here::i_am("docs/TGE-units.qmd")
target.dir <- "sandbox"
trop_glaz <- read_sf(here::here(target.dir,"trop-glacier-groups-labelled.gpkg"))
load(here::here(target.dir,"bioclim-model-data-groups.rda"))
```
...
```{r}
bioclim_dat <- trop_glaz_names %>% left_join(rawdata,by="id") %>% filter(!group_name %in% "Temperate Glacier Ecosystems")
bioclim_tbl <- bioclim_dat %>% group_by(Unit=group_name) %>%
  summarise(`Nr. of cells`=n(),
            `Glacier cells`=sum(glacier),
            ) 
...
slc <- bioclim_tbl %>% filter(`Glacier cells`>100) %>% pull(Unit)

pbio01 <- ggplot(bioclim_dat %>% filter(group_name %in% slc) %>% mutate(bio01=bio10_01/10)) + geom_histogram(aes(x=bio01,fill=glacier), bins=25) +

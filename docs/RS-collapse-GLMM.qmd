...
```{r}
model_data %>% group_by(unit,method) %>% summarise(collapse=sum(RS>.99)) %>% pivot_wider(values_from=collapse,names_from=method)
valid_methods <- c("ice","ess","ppv")
model_data <- model_data %>% filter(method %in% valid_methods) %>% mutate(method=droplevels(method))
```

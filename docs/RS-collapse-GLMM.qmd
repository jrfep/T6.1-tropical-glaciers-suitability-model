...
ggplot(model_data) +
  geom_boxplot(aes(x=RS,y=unit,color=method,group=interaction(unit,method)))

model_data_ss <- model_data %>% 
  filter(RS < 1, !unit %in% "Kilimanjaro") %>%
  mutate(
    period = factor(time),
...
mod_degradation_sat <- 
  glmmTMB(RS ~ period * scenario * method + (1|andes/unit/method), 
    dispformula=~method, 
    data = model_data_ss, 
    family=beta_family,
    REML=FALSE)
...
```{r}
#| echo: true
mod_degradation_simres<-simulateResiduals(mod_degradation_disp)
...
```{r}
model_data_ss$pred <- predict(mod_degradation_disp,type="response")

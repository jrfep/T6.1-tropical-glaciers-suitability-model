...
We can use an total ($RS_{ice}^{CT=0}$) or mean value ($RS_{bcs}^{CT}$) to describe the overall magnitude of decline, but the values of relative severity vary across any sample of sites or locations. 
...
For the purpose of describing these patterns of degradation, we calculate extent of decline ($ED_{RS≥x}$) as the proportion of the distribution of an assessment unit that has values of relative severity higher than a threshold $x$.

We will look at the calculation of ($ED_{RS≥x}$) for the two indicator variables and the relationship between RS and extent of decline through time.
...
## Extent of decline in bioclimatic suitability

We can use a similar approach to calculate the value of $ED_{RS≥x}$ for the decline in bioclimatic suitability. 
...
#| fig-cap: Comparison of the modified empirical cumulative distribution function of the $RS_{cor}$ values for the Tropical Glacier Ecosystem of Ecuador . 
